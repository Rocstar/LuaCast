-- Header Info
LRDMRC07={
Name="Lair Reive Drone Marjami Ravine C-7",
Author="Rockstar",
...
-- Cast Stone
function s()
local m={"Avian Roost"};for i,v in pairs(m)do 
if(t()==nil)or(t()~=v)then AshitaCore:GetDataModule():SendCommand('/target ' .. 17866998,0);
...
-- On Load
function LRDMRC07:OnLoad()
timer.Create("s",3.1,0,s);events.Add("onHandleNewChatLine","plugin_C_event",C);end

-- On Unload
function LRDMRC07:OnUnload()
timer.RemoveTimer('s');events.Remove("onHandleNewChatLine","plugin_C_event");end return LRDMRC07;
